...
public class BudgetDialog extends JDialog
{
	...
	JLabel hospitalFundRequest = new JLabel();
	JLabel hospitalFundAlloc = new JLabel();
	JSlider hospitalFundEntry;
	...
	private void applyChange()
	{
		...
		int newHospitalPct = ((Number) hospitalFundEntry.getValue()).intValue();
		...
		engine.hospitalPercent = (double)newHospitalPct / 100.0;
	...
	private void loadBudgetNumbers(boolean updateEntries)
	{
		...
		hospitalFundEntry.setValue((int)Math.round(b.hospitalPercent*100.0));
		}
		...
		hospitalFundRequest.setText(formatFunds(b.hospitalRequest));
		hospitalFundAlloc.setText(formatFunds(b.hospitalFunded));
	}
	...
	public BudgetDialog(Window owner, Micropolis engine)
	{
		...
		this.origHospitalPct = engine.hospitalPercent;
		...
		hospitalFundEntry = new JSlider(JSlider.HORIZONTAL, 0, 100, 100);
		adjustSliderSize(hospitalFundEntry);
		...
		hospitalFundEntry.addChangeListener(change);
	...
	private JComponent makeFundingRatesPane()
	{
		...
		fundingRatesPane.add(new JLabel(strings.getString("budgetdlg.hospital_fund")), c0);
		fundingRatesPane.add(hospitalFundEntry, c1);
		fundingRatesPane.add(hospitalFundRequest, c2);
		fundingRatesPane.add(hospitalFundAlloc, c3);
	...
	private void onResetClicked()
	{
		...
		engine.hospitalPercent = this.origHospitalPct;
		loadBudgetNumbers(true);
	}
